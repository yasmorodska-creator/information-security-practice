...
def seed():
    ...
    try:
        ...
        admin_user = User(username="admin", email="admin@university.edu", full_name="Адмін", password_hash="admin123", is_active=True)
        admin_user.roles.append(admin)
        teacher_user = User(username="petrov", email="petrov@university.edu", full_name="Петров І.С.", password_hash="teacher123")
        teacher_user.roles.append(teacher)
        student_user = User(username="ivanov", email="ivanov@university.edu", full_name="Іванов О.П.", password_hash="student123", group_id=group.id)
